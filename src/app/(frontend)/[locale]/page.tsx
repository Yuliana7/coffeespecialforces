...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [home, projects, workAreas, locations, latestEvents] = await Promise.all([
    ...
    payload.find({ collection: 'work-areas', locale, depth: 1, limit: 4, sort: 'order' }),
    payload.find({ collection: 'locations', locale, depth: 1, limit: 2 }),
    payload.find({ collection: 'events', locale, depth: 0, limit: 4, sort: '-date' }),
  ])
    ...
    <>
      <section className="relative isolate overflow-hidden bg-olive-deep text-white">
        ...
        <Container className="py-20 sm:py-28">
          ...
          {home.heroSubheading ? (
            <p className="mt-5 max-w-xl text-lg text-white/80">{home.heroSubheading}</p>
          ) : null}
      ...
      {home.aboutText ? (
        <Container className="py-16">
          {home.aboutHeading ? (
            <h2 className="mb-5 text-3xl uppercase sm:text-4xl">{home.aboutHeading}</h2>
          ) : null}
      ...
      {workAreas.docs.length > 0 ? (
        <Container className="py-16">
          <SectionHeading href={`/${locale}/work-areas`} linkLabel={t.nav_workAreas}>
            {t.nav_workAreas}
          </SectionHeading>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {workAreas.docs.map((area) => (
              ...
              >
                <h3 className="text-lg uppercase group-hover:text-blue">{area.title}</h3>
                {area.description ? (
                  <p className="mt-2 line-clamp-3 text-sm text-ink-muted">{area.description}</p>
                ) : null}
              </Link>
            ))}
          </div>
      ...
      {locations.docs.length > 0 ? (
        <Container className="py-16">
          <SectionHeading href={`/${locale}/locations`} linkLabel={t.nav_locations}>
            {t.nav_locations}
          </SectionHeading>
          <div className="grid gap-6 sm:grid-cols-2">
            {locations.docs.map((location) => (
              <div key={location.id} className="rounded-lg border border-line bg-surface p-6">
                <h3 className="text-xl uppercase">{location.name}</h3>
                {location.address ? (
                  <p className="mt-1 text-sm text-ink-muted">{location.address}</p>
                ) : null}
                {location.openingHours ? (
                  <p className="mt-3 whitespace-pre-line text-sm">{location.openingHours}</p>
                ) : null}
              </div>
            ))}
          </div>
